  ...
  app.get('/profile', isLoggedIn, function (req, res) {
    db.collection('orders')
      ...
      .toArray((err, result) => {
        ...
        const completedOrders = result.filter(({ completed }) => completed); // saves all the orders from the database with a true completed property
    ...
    db.collection('orders').save(
      {
        ...
        note,
        size,
        pending: true, // The default state for all the orders in the data base
        completed: false, // This is turned to true in the profile.ejs
      },
      (err, result) => {
        if (err) return console.log(err);
